...
def generate_word2vec_embeddings(sentences, model_name, batch_size):
    '''
    Creates embeddings for all sentences using a pre-trained word2vec model.
    '''
    ...
    return pd.DataFrame(concept_embeddings, columns=[f"dim{i}" for i in range(model.vector_size)])
...
def create_subspace(sentences_list, sentences_to_include, remove_stopword, batch_size):
    ...
    embeddings = generate_word2vec_embeddings(sentences, 'glove-wiki-gigaword-300', batch_size) # word2vec-google-news-300 100 billion words from google news 
...
combinations = list(itertools.product(sentences_var, remove_stopword))
...
for j,i in enumerate(combinations):
    ...
    file = f"new_subspaces_word2vec2/{i[0]}&{str(i[1])}.csv"
